<div class="hd-categories-badges" id="hd-form-badges" style="margin:10px 0; text-align:center;">
    @if(count($preferences) > 0)
        @foreach($preferences as $pref)
            <div data-id="{{ $pref->id }}" data-url="{{ route('user.profile.peference.destroy', [$profile->id, $pref->id]) }}"  class="hd-badge hd-badge-load-{{ $pref->id }}" data-item="Preference" data-container="hd-profile-preferences" data-loader="{{ route('user.profile.peferences.load', $profile->id) }}">
                @csrf
