		...
		<div class="row align-items-center justify-content-center hd-cover-small">
			<div class="col-lg-10">
				<div class="generic-banner-content">
					<h1 class="text-white" style="text-transform:uppercase;">Add Blog Preferences</h1>
                    <p class="text-white">These are categories and subcategories that define and categorize your blog. They will be usefull in search mode</p>
				</div>							
			</div>
        </div>
    </div>
...
<section class="hd-add-new">
    <div class="hd-add-form" style="margin-bottom:30px;">
        <div style="width:90%; margin:auto;" id="hd-profile-preferences">
            ...
            <script>loadHTMLAjax('hd-profile-preferences', "{{ route('blog.preferences.load', $blog->id) }}");</script>
        </div>
        <div class="text-center">
            ...
            <br/>
            <a href="{{ route('user.blog.show', [$blog->id, $blog->uuid]) }}" id="hd-profile-add-files-lnk" style="margin-top:15px;" class="genric-btn default-border radius">VIEW BLOG</a>
        </div>
    </div>
</section>
    ...
    <div class="row-col h-v">
        <div class="row-cell v-m">
            <div class="modal-dialog">
                <div class="modal-content" id="hd-preferences-body" url-check="{{ route('blog.preferences.check', $blog->id) }}" url-submit="{{ route('blog.preferences.store', $blog->id) }}" url-load="{{ route('blog.preferences.load', $blog->id) }}" data-id="{{ $blog->id }}">
                    <div class="modal-header">
                        <h5 class="modal-title">Add Blog Peferences</h5>
                    </div>
