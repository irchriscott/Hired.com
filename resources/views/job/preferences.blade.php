		...
		<div class="row align-items-center justify-content-center hd-cover-small">
			<div class="col-lg-10">
				<div class="generic-banner-content">
					<h1 class="text-white" style="text-transform:uppercase;">Add Job Preferences</h1>
                    <p class="text-white">These are categories and subcategories that define the job job or the service. They will be usefull in search mode</p>
				</div>							
			</div>
        </div>
    </div>
...
<section class="hd-add-new">
    <div class="hd-add-form" style="margin-bottom:30px;">
        <div style="width:90%; margin:auto;" id="hd-profile-preferences">
            ...
            <script>loadHTMLAjax('hd-profile-preferences', "{{ route('job.preferences.load', $job->id) }}");</script>
        </div>
        <div class="text-center">
            ...
            <br/>
            <a href="{{ route('job.images.create', [$job->id, $job->uuid]) }}" id="hd-profile-add-files-lnk" style="margin-top:15px;" class="genric-btn default-border radius">ADD JOB IMAGES</a>
        </div>
    </div>
</section>
    ...
    <div class="row-col h-v">
        <div class="row-cell v-m">
            <div class="modal-dialog">
                <div class="modal-content" id="hd-preferences-body" url-check="{{ route('job.preferences.check', $job->id) }}" url-submit="{{ route('job.preferences.store', $job->id) }}" url-load="{{ route('job.preferences.load', $job->id) }}" data-id="{{ $job->id }}">
                    <div class="modal-header">
                        <h5 class="modal-title">Add Job Peferences</h5>
                    </div>
