		...
		<div class="row align-items-center justify-content-center hd-cover-small">
			<div class="col-lg-10">
				<div class="generic-banner-content">
					<h1 class="text-white" style="text-transform:uppercase;">JOBS</h1>
                    <p class="text-white">View all posted job and filter them</p>
				</div>							
			</div>
        </div>
    </div>
...
<section class="hd-jb-container">
    <div class="hd-filter">
        <input type="search" id="hd-filter-data-input" data-url="{{ route('job.jobs') }}?type=all" data-type="all" class="form-control" placeholder="Search For Job Here At Hired.com"/>
        <div class="hd-filter-categories" id="hd-filter-categories">
            ...
            <div class="hd-categories" id="hd-categories">
                <ul>
                    <li class="active" data-url="{{ route('job.jobs') }}?type=all" data-type="all">
                        <div class="hd-image"><p><i class="fa fa-asterisk"></i></p></div>
                        <div class="hd-text"><p>All</p></div>
                    </li>
                    @if(count($categories) > 0)
                        @foreach($categories as $category)
                            <li data-type="{{ $category->id }}" data-url="{{ route('job.jobs') }}?type={{ $category->id }}">
    ...
    <div class="hd-jb-data">
        <div id="hd-jb-container-data">
            ...
            <script>loadHTMLAjax("hd-jb-container-data", "{{ route('job.jobs') }}?type=all");</script>
        </div>
    </div>
</section>
@endsection